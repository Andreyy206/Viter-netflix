...
;
  ...
  const mutation = useMutation({
    mutationFn: (values) => queryData( itemEdit ? `/v1/movies/${itemEdit.movies_aid}` : `/v1/movies`, itemEdit ? "put" : "post", values),
    onSuccess: (data) => {
            ...
            <Form>
              <div className='modal-main absolute top-0 right-0 h-screen w-[320px] bg-secondary grid grid-row-[auto,_1fr,_auto]'>
                ...
                <div className='modal-action flex justify-end gap-3 items-center p-3 px-4 pb-0 '>
                  <button
                    className="btn btn-accen" type="submit"
                    disabled={mutation.isPending}
                  >
                    {mutation.isPending ? <SpinnerButton /> : "Save"}
                  </button>
                  <button
                    className="btn btn-cancel" type="reset"
                    onClick={handleClose}
                  >
                    Cancel
                  </button>
